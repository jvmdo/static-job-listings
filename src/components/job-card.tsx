...
function JobCard({ job, isLoading, className, ...delegated }: JobCardProps) {
  return (
    <article
      {...delegated}
      className={twMerge(
    ...
    >
      ...
      <div className="grow shrink-0 flex flex-col justify-between gap-3 lg:gap-1">
        ...
        <ul className="flex items-center gap-4 pb-3 border-b border-secondary lg:border-none lg:pb-0 lg:-mt-1">
          <JobMeta>
            {<time dateTime={job.postedDate}>{job.postedAt}</time>}
          </JobMeta>
          <JobMeta>{job.contract}</JobMeta>
          <JobMeta>{job.location}</JobMeta>
        </ul>
      </div>
      <div className="flex flex-wrap gap-4 lg:pl-6 lg:justify-end lg:ml-auto">
        <JobTag category="role" value={job.role} />
        <JobTag category="level" value={job.level} />
